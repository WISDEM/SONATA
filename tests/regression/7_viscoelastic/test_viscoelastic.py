...
class TestVisco(unittest.TestCase):

    def test_one_viscoelastic(self):
        """
        ...
        npt.assert_allclose(tot_prony[:3], stiff[0][:3], atol=1e-5,
                            err_msg="First 3 rows of viscoelastic do not add to match the elastic.")

        npt.assert_allclose(tot_prony[3:], stiff[0][3:], atol=0.0002*tot_prony[3:].max(),
        ...
        npt.assert_allclose(prony_stiff[0][0][mask] / tot_prony[mask], 1./21.,
                            err_msg="Ratio of viscoelastic terms does not match ratio of inputs.")

    def test_two_viscoelastic(self):
        """
        ...
        visco_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), visco_file)
    ...
    def test_ortho_viscoelastic(self):
        ...
        elastic_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), elastic_file)
        ...
        npt.assert_allclose(tot_prony[:3], stiff[0][:3], rtol=1e-2,
                            err_msg="First 3 rows of viscoelastic do not add to match the elastic.")
